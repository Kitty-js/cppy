...
class Logger:
    def __init__(self, verbosity=False) -> None:
    ...
    def log(self, message, file=stdout):
        if self.verbose:
            print(f"\033[0;32m{message}\033[0m", file=file)

    def warn(self, message, file=stderr):
        print(f"WARNING: {message}", file=file)

    def error(self, message, file=stdout):
...
def copy_directory(src_dir: Path, dest_dir: Path, override=False, interactive=False):
    for src_child in src_dir.iterdir():
        ...
        if src_child.is_dir():
            ...
            logger.error(
                f"Skipping {src_child} because file type is not supported")
...
def copy(src: Path, dest: Path, override=False, recursive=False, interactive=False):
    if src.is_file():
        ...
        if not recursive:
            raise CpError(
                f"Skipping Directory {src} because -r is not present")
        if dest_is_dir:
            dest = dest / src.name
...
def cli():
    ...
    group.add_argument(
        "-i", "--interactive",
        action="store_true",
        help="Give details about actions being perfomed"
    )
    parser.add_argument(
        "-r", "--recursive",
        action="store_true",
